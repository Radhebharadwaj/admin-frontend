...
export default function UniversitiesClient() {
  ...
  const openEdit = (u: University) => {
    ...
    setFormData({
      ...
      logo_url: u.logo_url,
      search_aliases: u.search_aliases || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.logo_file) {
        const targetSlug = formData.slug || autoSlug(formData.acronym || formData.name || "");
      ...
      // Build strictly clean JSON payload — no spread, no File objects
      const safePayload = {
        ...
        search_aliases: Array.isArray(formData.search_aliases)
          ? (formData.search_aliases as unknown as string[]).join(", ")
      ...
      const optimisticData = editingId
        ? universities.map((u) => (u.id === editingId ? { ...u, ...safePayload } : u))
        : [{ ...safePayload, id: `temp-${Date.now()}`, logo_url: safePayload.logo_url || null } as University, ...universities];
      ...
      await mutate(
        async () => {
          ...
          // Re-fetch to get accurate DB values
    ...
    <div className="max-w-7xl mx-auto pb-20 animate-in fade-in duration-500">
      ...
      <div className="relative max-w-sm w-full mb-6">
        ...
        <input
          ...
          className="w-full bg-zinc-900 border border-zinc-800 rounded-xl pl-10 pr-10 py-2.5 text-sm text-zinc-200 placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-zinc-700 transition-all"
        />
        {search && (
          <button
            onClick={() => setSearch("")}
